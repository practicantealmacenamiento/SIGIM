...
class Question:
    ...
    def _validate_invariants(self) -> None:
        ...
        if self.type == "choice" and not self.choices:
            raise ValidationError(message="Las preguntas de tipo 'choice' deben tener al menos una opción.", field="choices")

        if self.type != "choice" and self.choices:
            raise ValidationError(message=f"Las preguntas de tipo '{self.type}' no pueden tener opciones.", field="choices")
...
class Answer:
    ...
    # Mutating updaters (used by services)
    ...
    # Immutable-style helpers (still used in some call sites)
    ...
    def has_content(self) -> bool:
        return bool(self.answer_text or self.answer_choice_id or self.answer_file_path or (self.meta and len(self.meta) > 0))
