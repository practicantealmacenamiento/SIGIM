 ...
 * - Lee `questionnaire_id` y `submission_id` desde props o querystring.
 * - Se apoya en `useFormFlow` para orquestar items, envío, OCR y navegación.
 * - Ofrece un modal de “Resumen” y una barra fija de acciones.
 */

// ========= Config =========
const Q_FASE1 = (process.env.NEXT_PUBLIC_Q_FASE1_ID as string) || null;

// ========= Helpers generales (sin cambio de lógica) =========
...
// Nota: se mantienen tal cual tus estrategias de fallback
...
function FormularioContent(props: {
  ...
  // Normaliza IDs (prop > querystring)
  const qid = useMemo(
  ...
  // Hook central de flujo del formulario
  const {
    submissionId,
    items,
    setItems,
  ...
  // —— Sincronización de querystring con qid ——
  useEffect(() => {
    if (!qid) return;
  ...
  // —— Sincronización de querystring con submissionId ——
  useEffect(() => {
  ...
  }, [submissionId, qid, router, search]);

  // —— Abrir editor en índice específico (scroll suave al elemento) ——
  const openEditorAt = useCallback((idx: number) => {
    ...
    setItems((prev) => {
      ...
      const qidStr = String((prev[idx] as any).q.id);
      // Scroll suave al item abierto
      setTimeout(() => {
        const el = document.querySelector<HTMLElement>(`[data-qid="${qidStr}"]`);
        el?.scrollIntoView({ behavior: "smooth", block: "center" });
      }, 50);
      return next;
    });
  }, [setItems, setMostrarResumen]);

  // —— Draft detectado: CTA para continuar o reiniciar ——
  if (resumeDraft) {
    return (
  ...
  // —— Falta qid: CTA para elegir/usar Fase 1 ——
  if (!qid) {
    return (
        ...
        <div className="mt-6 space-y-6">
          {items.map((it, idx) => (
            <QuestionCard
              ...
              retryOCR={retryOCR}
              setActor={setActor}
              //Pasamos el error SOLO a la tarjeta activa para pintarlo ahí
...
// ========= Export página con Suspense (sin cambios) =========
